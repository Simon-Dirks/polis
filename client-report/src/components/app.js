            ...
            attemptResponse.then(
                (response) => {
                    if (response.status && response.status === 'pending') {
                        this.corMatRetries = _.isNumber(this.corMatRetries)
                            ? this.corMatRetries + 1
                            : 1
                        setTimeout(
                            () => {
    ...
    render() {
        ...
        if (this.state.loading) {
            return (
                <div className={'p-8'}>
                    <div>
                        <svg
                            className="animate-spin mr-3 h-5 w-5 text-black inline relative bottom-[3px]"
        ...
        return (
            <div className={'h-full'}>
                ...
                <div
                    className={
                        'md:p-8 md:pb-16 pt-12 md:pt-20 h-full overflow-y-auto max-w-screen-2xl mx-auto'
                    }
                >
                    {/*<Controls*/}
